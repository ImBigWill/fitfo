...
test("builds a client plan from scan, crawl, and research signals", () => {
  ...
  assert.deepEqual(plan.vertical, {});
...
test("renders a Markdown plan for Obsidian", () => {
  ...
  assert.doesNotMatch(markdown, /## Vertical Lens/);
  assert.doesNotMatch(markdown, /## Homeowner Emergency UX/);
  assert.doesNotMatch(markdown, /## Plumbing Proof Assets Needed/);
  assert.doesNotMatch(markdown, /## Plumbing Call Questions/);
